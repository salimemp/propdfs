    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          // GDPR / ePrivacy so users can change their preferences
          // after initially consenting. The banner itself reads
          // the pref key on every mount so flipping the flag
          // here causes the banner to re-appear the next time
          // the user navigates to a page that mounts it.
          _buildListTile(
            context,
            icon: Icons.tune,
            title: 'Manage cookie preferences',
            subtitle: 'Re-open the cookie consent banner',
            onTap: () async {
              final prefs = await SharedPreferences.getInstance();
              await prefs.setBool('cookie_consent_has_consented', false);
              if (!context.mounted) return;
              ref.read(cookieConsentProvider.notifier).acceptEssentialOnly();
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'Cookie preferences reset. The banner will re-appear '
                    'on your next screen.',
                  ),
